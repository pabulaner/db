...
int64_t row4 = 0;
cerr << "count: " << db.warehouse.count();
for (uint64_t i = 0; i != db.warehouse.count(); i++){ //257; virtual void Scan::produce(const IUSet&, ConsumerFn)
db.warehouse.scan({i}, [&](auto key, auto record){ //258; Scan::produce(const IUSet&, ConsumerFn)::<lambda()>
